...
public class UploadController {
    ...
    private String uploadPath;//application.properties 파일의 설정값을 읽어서 Injection
    ...
    public ResponseEntity<List<UploadResultDTO>> uploadFile(MultipartFile[] uploadFiles, String mid){
        ...
        for (MultipartFile uploadFile: uploadFiles) {
            ...
            String fileName = originalName.substring(originalName.lastIndexOf("\\") + 1);//마지막 슬래시 부분을 찾아내서 파일 이름을 추출
            ...
            try {            
                uploadFile.transferTo(savePath);//원본 파일 저장 >> 이 부분이 없로드          
                ...
                //-----------DB에 저장(하나의 멤버에 여러 개의 Profile를 가짐)---------
